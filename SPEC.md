...
1. [Structure](#structure)
   1. [Header](#header)
      1. [Flag table](#flag-table)
      2. [Version mismatch handling](#version-mismatch-handling)
   2. [Index](#index)
   3. [Data](#data)
...
|            8-9<br>10-11<br/>12-13            | Spec version: <br/>major, <br>minor, <br>patch | The spec version as three unsigned `short`s                                   | The version[^1] of the standard used to create the file. If this doesn't match the implementation's version the implementation should handle the mismatch according to the [Version mismatch handling](#version-mismatch-handling) table below. |
|                    14-17                     |                  Index length                  | one `uint32`                                                                  | The length of the [index](#index).                                                                                                                                                                                                              |
|                      18                      |                     Flags                      | *See [flag table](#flag-table) below.*                                        | If all flags are set the byte should have the value 0x83 (`0b10000011`).                                                                                                                                                                        |
...
|   `LOCKED`    |  Byte 14<br>Bit 5   | The database can only be opened in ['r' or 'rf' mode](#modes) and each writing action will cause an exception to be raised: `L2DBError: Database is locked!`.                                                                                                                                                     |
...
| Version | If lower                                                                                                                                                           | If higher       |
|:-------:|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------|
|  major  | Try to convert the DB, otherwise fail with `L2DBVersionMismatch` error with message `Major version mismatch ({{db_ver}} vs {{imp_ver}}). Conversion failed.`       | *Same as lower* |
|  minor  | Try to convert the DB, otherwise fail with `L2DBVersionMismatch` error with message `Minor version mismatch ({{db_ver}} vs {{imp_ver}}). Conversion failed.`       | *Same as lower* |
|  patch  | Emit a warning: `L2DB: Patch versions don't match ({{db_ver}} vs {{imp_ver}}). Be aware that certain clarifications in the spec can cause behavioral differences.` | *Same as lower* |
...
Note: type identifiers are ASCII-encoded!*
...
| Floating point number |   `flt`    | Any positive or negative 64-bit number. (aka.`double`[^2])                                                                                                                                                                                                                                                                                                                                                                                               |
